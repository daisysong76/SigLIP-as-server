#!/usr/bin/env python3
"""
Simple viewer for LLaVA reasoning results stored in JSON format.
"""
import json
import os
import sys
...
def main():
    parser = argparse.ArgumentParser(description="View LLaVA results for images from JSON file")
    parser.add_argument("--metadata", type=str, default="llava_reasoning.json",
                        help="Path to JSON metadata file with LLaVA results")
    ...
    try:
        ...
        print(f"Loaded JSON metadata from {args.metadata}")
    ...
    for i, entry in enumerate(entries):
        ...
        # Print basic info
        print(f"ID: {entry.get('id', f'Sample {i}')}")
        if 'caption' in entry:
            print(f"Caption: {entry['caption']}")
        
        # Show the image if requested
        if args.show_images and args.image_key in entry:
            img_path = entry[args.image_key]
            ...
            if os.path.exists(img_path):
                try:
                    ...
                    plt.imshow(img)
                    plt.title(entry.get('caption', 'No caption available'))
                    plt.axis('off')
                    plt.show()
                except Exception as e:
