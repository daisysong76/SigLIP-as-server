...
def main():
    ...
    parser.add_argument("--metadata", type=str, default="clip-service/scripts/all_image_metadata.pkl",
                        help="Path to metadata file with LLaVA results (JSON or pickle)")
    ...
    file_ext = os.path.splitext(args.metadata)[1].lower()
    try:
        if file_ext == '.json':
            ...
            print(f"Loaded pickle metadata from {args.metadata}")
